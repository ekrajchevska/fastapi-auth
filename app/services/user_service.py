...
class UserService:
    ...
    def create_login_verification_obj(db: Session, email: str, code: str):
        db_old_login = (
            db.query(models.LoginVerification)
            .filter(models.LoginVerification.email == email)
            .first()
        )
        if db_old_login is not None:
    ...
    def verify_login(db: Session, email: str, code: str):
        ...
        if code != db_login_obj.code:
            raise HTTPException(
                status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Invalid code!"
            )
